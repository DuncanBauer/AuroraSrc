...
{
	public:
		MapleAESOFB(byte * key, byte * iv, _short mapleVersion);
		~MapleAESOFB();

		void setIv(byte * iv);
		byte * getIv();
		void updateIv();
		void crypt(byte * data, int length);
		void getPacketHeader(int length, byte * ret);
		int getPacketLength(int packetHeader);
		bool checkPacket(byte * packet);
		bool checkPacket(int packetHeader);
		void getNewIv(byte * iv);
		std::string toString();
		void funnyShit(byte inputByte, byte * in);
		//byte[] funnyRamon(byte inputByte, byte[] in);

	private:
		byte * iv;
		//Cipher cipher;
		_short mapleVersion;
		byte funnyBytes[256] = {(byte) 0xEC, (byte) 0x3F, (byte) 0x77, (byte) 0xA4, (byte) 0x45, (byte) 0xD0, (byte) 0x71, (byte) 0xBF, (byte) 0xB7, (byte) 0x98, (byte) 0x20, (byte) 0xFC,
