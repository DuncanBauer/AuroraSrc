...
//    }
    ...
    this.setIv(iv);
    this.mapleVersion = (_short) (((mapleVersion >> 8) & 0xFF) | ((mapleVersion << 8) & 0xFF00));
}
...
byte[] MapleAESOFB::crypt(byte[] data) {
...
byte[] MapleAESOFB::getPacketHeader(int length) {
    int iiv = (iv[3]) & 0xFF;
    iiv |= (iv[2] << 8) & 0xFF00;
...
byte[] MapleAESOFB::getNewIv(byte oldIv[]) {
...
byte[] MapleAESOFB::funnyShit(byte inputByte, byte[] in) {
    byte elina = in[1];
    byte anna = inputByte;
...
byte[] MapleAESOFB::funnyRamon(byte inputByte, byte[] in) {
